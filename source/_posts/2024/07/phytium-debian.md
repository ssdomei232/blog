...
---
{% note warning %}
现在已经不再推荐使用下文的方式，更推荐使用由星火社区维护的发行版[GXDE](https://www.gxde.top/),对飞腾等国产芯片支持良好  
您可以在校园网联合镜像站下载GXDE的安装镜像: `https://mirrors.cernet.edu.cn/GXDE/ISO/`
{% endnote %}
...
4. 保存退出。
2. 修改 /etc/sudoers 文件属性为只读。这时就可以使用 sudo 命令了。
